    ...
    <div class="settings-container">
      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <q-toggle
            v-model="settings.snakeWrap"
            label="Wrap Walls"
            @update:model-value="saveSettings"
          />
          <div class="text-caption text-white q-mt-xs" style="opacity: 0.7;">
            When on, the snake passes through walls and reappears on the opposite side.
          </div>
        </q-card-section>
      </q-card>
      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Best score: {{ progressStore.snake.bestScore }}<br />
            {{ progressStore.snake.gamesPlayed }} games played
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your best score and game count. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetSnakeProgress()
    haptics.success()
  })
}
</script>
