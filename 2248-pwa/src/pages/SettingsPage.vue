      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Best score: {{ progressStore.game2248.bestScore }} ·
            Best tile: {{ progressStore.game2248.bestTile || '—' }}<br />
            {{ progressStore.game2248.gamesPlayed }} games played
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your best score, best tile and game count. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.reset2248Progress()
    haptics.success()
  })
}
</script>
