      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Best score: {{ progressStore.game2048.bestScore }} ·
            Best tile: {{ progressStore.game2048.bestTile || '—' }}<br />
            {{ progressStore.game2048.gamesWon }} wins in
            {{ progressStore.game2048.gamesPlayed }} games
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your best score, best tile and game counts. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.reset2048Progress()
    haptics.success()
  })
}
</script>
