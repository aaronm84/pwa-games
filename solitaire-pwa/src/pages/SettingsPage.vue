    ...
    <div class="settings-container">
      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-caption text-white q-mb-md" style="opacity: 0.8;">
            The table theme automatically changes throughout the day based on the time. However, feel free to change it to whatever theme fits your mood.
          </div>
      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <q-select
            v-model="settings.solitaireDealCount"
            :options="dealCountOptions"
            option-label="label"
            option-value="value"
            emit-value
            map-options
            label="Cards per deal"
            @update:model-value="saveSettings"
          />

          <q-toggle
            v-model="settings.solitaireHintsEnabled"
            label="Show Hints"
            class="q-mt-md"
            @update:model-value="saveSettings"
          />
      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            {{ progressStore.solitaire.gamesWon }} of
            {{ progressStore.solitaire.gamesPlayed }} games won
            ({{ progressStore.solitaireWinRate }}%)
          </div>
...
const dealCountOptions = [
  { label: 'Deal one (easier)', value: 1 },
  { label: 'Deal three (classic)', value: 3 },
]
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your wins, streaks and best times. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetSolitaireProgress()
    haptics.success()
  })
}
</script>
