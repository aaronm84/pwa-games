      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Total stars: {{ progressStore.ripples.totalScore }}<br />
            Level reached: {{ progressStore.ripples.currentLevel }} ·
            {{ progressStore.ripples.levelsCompleted }} ponds cleared ·
            {{ progressStore.ripples.perfectLevels }} perfect
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your stars and level progress. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetRipplesProgress()
    haptics.success()
  })
}
</script>
