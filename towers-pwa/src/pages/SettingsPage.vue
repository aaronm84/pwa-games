      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Best score: {{ progressStore.towers.bestScore.toLocaleString() }}<br />
            Best wave: {{ progressStore.towers.bestWave }} ·
            {{ progressStore.towers.gamesPlayed }} games
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your best score and wave progress. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetTowersProgress()
    haptics.success()
  })
}
</script>
