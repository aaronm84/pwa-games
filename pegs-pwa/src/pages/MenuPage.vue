  ...
  <q-page class="menu-page">
    ...
    <div class="menu-content">
      ...
      <div class="title-section">
        <h1 class="game-title">
          <span class="title-emphasis">Pegs</span>
        </h1>
        <p class="game-subtitle">Aim, drop, clear the orange</p>
      </div>
      ...
      <!-- Progress Info -->
      <div v-if="pegs.highestLevel > 1 || pegs.bestScore > 0" class="progress-info">
        <div class="progress-stat">
          <q-icon name="emoji_events" size="sm" color="yellow-6" />
          <span>Best score: {{ pegs.bestScore }}</span>
        </div>
        <div class="progress-stat">
          <q-icon name="stairs" size="sm" color="orange-6" />
          <span>Highest level: {{ pegs.highestLevel }}</span>
        </div>
      </div>
    </div>
  </q-page>
...
const { pegs } = storeToRefs(progressStore)
const haptics = useHaptics()

function playGame() {
  haptics.medium()
  router.push({ name: 'pegs' })
}
