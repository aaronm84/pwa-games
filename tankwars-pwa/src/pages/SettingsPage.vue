      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            {{ progressStore.tankwars.gamesWon }} of
            {{ progressStore.tankwars.gamesPlayed }} battles won<br />
            Win streak: {{ progressStore.tankwars.streak }} (best {{ progressStore.tankwars.bestStreak }})
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your battle record and streaks. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetTankProgress()
    haptics.success()
  })
}
</script>
