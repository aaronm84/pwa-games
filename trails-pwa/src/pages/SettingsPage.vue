      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            {{ progressStore.trails.gamesWon }} of
            {{ progressStore.trails.gamesPlayed }} games won<br />
            Win streak: {{ progressStore.trails.streak }} (best {{ progressStore.trails.bestStreak }})
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your win record and streaks. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetTrailsProgress()
    haptics.success()
  })
}
</script>
