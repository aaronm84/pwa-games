  ...
  <q-page class="menu-page">
    ...
    <div class="menu-content">
      ...
      <div class="title-section">
        <h1 class="game-title">
          <span class="title-emphasis">Bricks</span>
        </h1>
        <p class="game-subtitle">Bounce the ball, smash the wall</p>
      </div>
      ...
      <!-- Progress Info -->
      <div v-if="bricks.highestLevel > 1 || bricks.bestScore > 0" class="progress-info">
        <div class="progress-stat">
          <q-icon name="emoji_events" size="sm" color="yellow-6" />
          <span>Best score: {{ bricks.bestScore }}</span>
        </div>
        <div class="progress-stat">
          <q-icon name="stairs" size="sm" color="orange-6" />
          <span>Highest level: {{ bricks.highestLevel }}</span>
        </div>
      </div>
    </div>
  </q-page>
...
const { bricks } = storeToRefs(progressStore)
const haptics = useHaptics()

function playGame() {
  haptics.medium()
  router.push({ name: 'bricks' })
}
