      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Best score: {{ progressStore.wildcard.bestScore.toLocaleString() }}<br />
            Furthest ante: {{ progressStore.wildcard.bestAnte }} ·
            {{ progressStore.wildcard.runsWon }}/{{ progressStore.wildcard.runsPlayed }} runs won
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your best score and level progress. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetWildcardProgress()
    haptics.success()
  })
}
</script>
