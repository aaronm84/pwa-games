  ...
  <q-page class="menu-page">
    ...
    <div class="menu-content">
      ...
      <div class="title-section">
        <h1 class="game-title">
          <span class="title-emphasis">Wildcard</span>
        </h1>
        <p class="game-subtitle">A poker roguelike — hands, jokers, and rising stakes</p>
      </div>
        ...
        >
          <div class="btn-content">
            ...
            <div class="btn-text">
              ...
              <div class="btn-sublabel">Start from level 1</div>
            </div>
          </div>
        </q-btn>
      ...
      <!-- Progress Info -->
      <div v-if="wildcard.bestAnte > 0 || wildcard.bestScore > 0" class="progress-info">
        <div class="progress-stat">
          <q-icon name="emoji_events" size="sm" color="yellow-6" />
          <span>Best score: {{ wildcard.bestScore.toLocaleString() }}</span>
        </div>
        <div class="progress-stat">
          <q-icon name="trending_up" size="sm" color="green-6" />
          <span>Furthest: Ante {{ wildcard.bestAnte }}{{ wildcard.runsWon > 0 ? ` · ${wildcard.runsWon} won` : '' }}</span>
        </div>
      </div>
    </div>
  </q-page>
...
const { wildcard } = storeToRefs(progressStore)
const haptics = useHaptics()

function playGame() {
  haptics.medium()
  router.push({ name: 'wildcard' })
}
