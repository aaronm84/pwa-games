      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Highest level: {{ progressStore.flow.highestLevel }}<br />
            {{ progressStore.flow.levelsSolved }} levels solved
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    ...
    message: 'This permanently clears your highest level and solved count. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetFlowProgress()
    haptics.success()
  })
}
</script>
