  ...
  <q-page class="menu-page">
    ...
    <div class="menu-content">
      ...
      <div class="title-section">
        <h1 class="game-title">
          <span class="title-emphasis">Marbles</span>
        </h1>
        <p class="game-subtitle">Match the chain before it falls</p>
      </div>
      ...
      <!-- Progress Info -->
      <div v-if="marbles.highestLevel > 1 || marbles.bestScore > 0" class="progress-info">
        <div class="progress-stat">
          <q-icon name="emoji_events" size="sm" color="yellow-6" />
          <span>Best score: {{ marbles.bestScore }}</span>
        </div>
        <div class="progress-stat">
          <q-icon name="stairs" size="sm" color="green-6" />
          <span>Highest level: {{ marbles.highestLevel }}</span>
        </div>
      </div>
    </div>
  </q-page>
...
const { marbles } = storeToRefs(progressStore)
const haptics = useHaptics()

function playGame() {
  haptics.medium()
  router.push({ name: 'marbles' })
}
