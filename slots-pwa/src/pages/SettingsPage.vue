      ...
      <q-card class="settings-card q-mb-md">
        <q-card-section>
          ...
          <div class="text-body2 text-white q-mb-md" style="opacity: 0.9;">
            Credits: {{ progressStore.casino.credits.toLocaleString() }}<br />
            Best win: {{ progressStore.casino.bestWin.toLocaleString() }} ·
            {{ progressStore.casino.spins.toLocaleString() }} spins
          </div>
...
function confirmReset() {
  ...
  $q.dialog({
    title: 'Reset Casino',
    message: 'This resets your credits, jackpot, and stats back to the start. Continue?',
    cancel: true,
    persistent: true,
    dark: true,
  }).onOk(async () => {
    await progressStore.resetCasino()
    haptics.success()
  })
}
</script>
